...
*Script developed by Eugene's AI — Fractional CFO & Scaling Advisory*`
...
- Don't sit behind a desk — it creates physical and psychological distance
    ...
    **Eugene's rule:** If your team isn't trained on value-selling, you're giving away 30% of your margins.`,
  },
  {
    num: 2,
...
Then pause. Don't speak first. The first person to talk after stating a price is negotiating against themselves.
    ...
    **Eugene's benchmark:** If >30% of patients say yes to your first offer without objection, your pricing structure is under-modeled.`,
  },
  {
    num: 5,
...
People don't say "neither." They pick one. This is the most powerful rebooking tool in the industry.
    ...
    **Eugene's stat:** Practices that rebook at checkout see 75%+ rebooking rates, directly compounding lifetime value.`,
  },
]
    ...
    <div className="space-y-5">
      ...
      {loading && !output && (
        <div className="flex justify-center py-8">
          <EugeneTyping message="Eugene is writing your script..." />
        </div>
      )}
    </div>
  )
      ...
      <div className="space-y-3 max-h-96 overflow-y-auto p-1">
        {messages.map((m, i) => (
          <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`max-w-lg rounded-2xl px-4 py-3 text-sm ${m.role === 'jamie' ? 'chat-message-eugene' : 'chat-message-user'}`}>
        ...
        {streaming && streamMsg && (
          <div className="flex gap-3">
            ...
            <div className="max-w-lg rounded-2xl px-4 py-3 text-sm chat-message-eugene">
              <pre className="whitespace-pre-wrap font-body text-gray-800">{streamMsg}</pre>
            </div>
          </div>
        )}
        {streaming && !streamMsg && <div className="flex gap-3"><div className="w-7 h-7 rounded-full bg-rose-med/20 flex items-center justify-center text-sm">🎭</div><EugeneTyping message="Jamie is typing..." /></div>}
      </div>
