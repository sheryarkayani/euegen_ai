...
import { CheckCircle, Circle, ChevronDown, ChevronRight, MessageCircle, Calculator } from 'lucide-react'
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <Badge variant="indigo">{totalChecked}/{totalItems} complete</Badge>
      ...
      {showCostCalc && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <Card>
            ...
            <div className="mb-4">
              <label className="text-xs text-gray-500 mb-2 block font-medium">Location size modifier</label>
              <div className="flex gap-2">
                {[{ label: 'Boutique (1,200 sq ft)', val: 0.7 }, { label: 'Standard (1,800 sq ft)', val: 1 }, { label: 'Premium (2,500 sq ft)', val: 1.4 }].map(o => (
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-xs font-medium transition-all ${costMultiplier === o.val ? 'btn-primary text-white shadow-sm' : 'bg-gray-100 text-gray-500 hover:text-navy-950'}`}
                  >
                    {o.label}
                  </button>
                ))}
            ...
            <div className="space-y-2">
              {STARTUP_COSTS.map(c => (
                <div key={c.category} className="flex items-center justify-between py-2 border-b border-gray-100">
              ...
              <div className="flex items-center justify-between py-3 border-t border-indigo-100 mt-2">
            ...
            <p className="text-xs text-gray-500 mt-3">* Ranges reflect market variation across US metro areas. Urban markets (NYC, LA, Miami) run 30–50% higher.</p>
          </Card>
        </motion.div>
      )}
                ...
                <div className="mt-4 space-y-2">
                  {phase.items.map(item => (
                    <motion.div
                      ...
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className={`checklist-item flex items-start justify-between gap-3 p-3 rounded-xl ${checked[item.id] ? 'checked bg-gray-50/50' : 'bg-white'}`}
                    ...
                    >
                      <div className="flex items-start gap-3 flex-1">
                        ...
                        <p className={`text-sm leading-relaxed ${checked[item.id] ? 'line-through text-gray-400 font-normal' : 'text-gray-700 font-medium'}`}>
                          {item.text}
                        </p>
                      </div>
                      <button
